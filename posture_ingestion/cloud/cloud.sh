...
docker run -d --name readandsave_slouch -v /mnt/hw3bucket:/data --network cloudbridge readandsave_slouch
docker run -d --name readandsave_straight -v /mnt/hw3bucket:/data --network cloudbridge readandsave_straight
